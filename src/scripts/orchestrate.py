...
#
# NOTE one account can own several key files, which is why the pool dedupes by username.
...
def build_queue(stage: str | None = None) -> list[dict]:
    ...
    if stage == "seedmatch":
        # GlucoPRISM-C has 6 seeds; every comparator has 3, and C's seeds 3-5
        # came in better than 0-2 in all four conditions. Under matched seeds C
        # ranks 4th, not 1st, and the Holm-corrected p moves 0.021 -> 0.055.
        # The comparison must be seed-matched before anything is claimed, and
        # the baseline everything is measured against is the one that matters.
        q = [{"id": f"W3u-ov40-s{s}", "stage": "SEED", "model": "glucofm",
              "script": "run_pretrain.py", "seed": s,
              "extra": ["--shard-suffix", "_ov40"]} for s in (3, 4, 5)]
        for s in (3, 4, 5):
            q.append({"id": f"E-v2-vib-simbias-s{s}", "stage": "SEED",
                      "model": "v2port", "script": "run_v2port.py", "seed": s,
    ...
    if stage == "capacity":
        # Reviewer asked whether the addressability claim is really about the
        # reserved block or just about regularization: sweep how much the block
        # can hold (its width) and how dearly it pays per nat (the KL weight).
        # Released model is d_sensor=16, w_vib=0.1, so those cells already exist
        # and are not re-run. Two seeds per arm.
        base = ["--corpus", "corpus_v2fmt_ov40.npz", "--use-vib"]
        q = []
        for s in (0, 1):
            for d in (8, 32):
                q.append({"id": f"K-dA{d}-s{s}", "stage": "CAP", "model": "v2port",
                          "script": "run_v2port.py", "seed": s,
                          "extra": [*base, "--w-vib", "0.1", "--d-sensor", str(d)]})
            for b in ("0.03", "0.3", "1.0"):
                tag = b.replace(".", "")
                q.append({"id": f"K-beta{tag}-s{s}", "stage": "CAP",
                          "model": "v2port", "script": "run_v2port.py", "seed": s,
                          "extra": [*base, "--w-vib", b]})
        return q

    if stage == "confound":
        # Separates the v2 components from the factorization. Two arms:
        #   X  components only, protocol objectives OFF
        #   Y  components + VIB, protocol objectives OFF
        # Against the released C (components + VIB + protocol objectives ON),
        # these say how much of the +2.63 is the block structure at all.
        base = ["--corpus", "corpus_v2fmt_ov40.npz", "--no-protocol"]
        q = []
        for s in (0, 1, 2):
            q += [{"id": f"X-noproto-s{s}", "stage": "CONF", "model": "v2port",
                   "script": "run_v2port.py", "seed": s, "extra": list(base)},
                  {"id": f"Y-noproto-vib-s{s}", "stage": "CONF", "model": "v2port",
                   "script": "run_v2port.py", "seed": s,
                   "extra": [*base, "--use-vib", "--w-vib", "0.1"]}]
        return q
...
def package(user: str) -> Path:
    ...
    shutil.copytree(ROOT / "src" / "glucoprism", out / "src" / "glucoprism",
