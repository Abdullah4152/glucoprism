...
from glucoprism.eval.probe import _metrics                # noqa: E402
...
def build_glucofm(ck: Path):
    from glucoprism.models.glucofm import GlucoFM, GlucoFMConfig
...
def build_cgm_jepa(ck: Path):
    from glucoprism.models.cgm_jepa import Encoder
    blob = torch.load(ck, map_location="cpu", weights_only=False)
    cfg = blob["cfg"]
