...
# ------------------------------------------------------- HPP external cohort
# Derived from hpp_results.csv rather than typed, on the same principle as every
# other value here. That CSV is the only thing that leaves the Pheno.AI TRE:
# model x task x metric aggregates, no participant data. Cohort counts come from
# hpp_meta.json when the enclave export is present, and fall back to the figures
# printed by the cohort build otherwise.
_HPP_P = A / "hpp_results.csv"
if _HPP_P.exists():
    _H = pd.read_csv(_HPP_P)

    def _hm(model: str, metric: str = "roc_auc", split: str = "cv",
    ...
    # One decimal, always: `{v:g}` would print 66.0 as "66", which reads as a
    # different precision from the 68.5 beside it in the same sentence.
    def _f1(x: float) -> str:
        return f"{x:.1f}"

    _fm, _iglu = _hm("glucofm"), _hm("iglu")
    _ce, _cc = _hm("glucoprism_e__zTzS"), _hm("glucoprism_c__zTzS")
    C.update({
        "hpp_auce": _f1(_ce),
        "hpp_aucc": _f1(_cc),
        "hpp_fm": _f1(_fm),
        "hpp_iglu": _f1(_iglu),
        "hpp_trunc": _f1(_hm("glucofm__trunc112")),
        # Mean glucose on the DIABETES endpoint specifically: this is the number
        # the label audit predicted (0.654) and the trivial baseline the prose
        # cites. Averaged over all three endpoints it would be 58.5 and would
        # not correspond to anything claimed.
        "hpp_meanauc": _f1(_hm("cgm_mean", task="diabetes")),
        "hpp_delta": _f1(_ce - _fm),
        "hpp_igludelta": _f1(_ce - _iglu),
        "hpp_deltapr": _f1(_hm("glucoprism_e__zTzS", "pr_auc")
                           - _hm("glucofm", "pr_auc")),
        "hpp_deltatr": _f1(_hm("glucoprism_e__zTzS", split="transfer")
                           - _hm("glucofm", split="transfer")),
        # The intervention: the same encoder read with and without zA.
        "hpp_dropc": _f1(_cc - _hm("glucoprism_c__full")),
        "hpp_drope": _f1(_ce - _hm("glucoprism_e__full")),
        "hpp_droptr": _f1(_hm("glucoprism_c__zTzS", split="transfer")
                          - _hm("glucoprism_c__full", split="transfer")),
        # Trait vs State vs Sensor, the separation claim.
        "hpp_traitstate": _f1(_hm("glucoprism_c__zT") - _hm("glucoprism_c__zS")),
        "hpp_sensorgap": _f1(_hm("glucoprism_c__zT") - _hm("glucoprism_c__zA")),
        # The Sensor gap is not uniform: it is widest on the one endpoint that
        # is a subject-stable trait, and widens further under the device shift.
        # That pattern is the claim, so both numbers are macros.
        "hpp_sensorgapob": _f1(_hm("glucoprism_c__zT", task="obesity")
                               - _hm("glucoprism_c__zA", task="obesity")),
        "hpp_sensorgaptr": _f1(_hm("glucoprism_c__zT", split="transfer",
                                   task="obesity")
                               - _hm("glucoprism_c__zA", split="transfer",
                                     task="obesity")),
        "hpp_obe": _f1(_hm("glucoprism_e__zTzS", task="obesity")),
        "hpp_obiglu": _f1(_hm("iglu", task="obesity")),
    })
    _n = _H[(_H.split == "cv") & (_H.metric == "roc_auc")].set_index("task").n
    for _t, _k in (("diabetes", "hpp_dr"), ("hyperlipidemia", "hpp_hl"),
                   ("obesity", "hpp_ob")):
        C[_k] = int(_n.loc[_t].iloc[0])

    _meta_p = A / "hpp_meta.json"
    _meta = json.loads(_meta_p.read_text()) if _meta_p.exists() else {}
    C["hpp_subj"] = int(_meta.get("subjects", 9303))
    C["hpp_wins"] = int(_meta.get("windows", 95445))
    C["hpp_singledev"] = int(_meta.get("transfer", {}).get("single_device", 9251))
    # Measured during label construction: mean glucose against the assayed HbA1c.
    # Not derivable from the results CSV, and the number the non-circularity of
    # the diabetes label rests on.
    C["hpp_earr"] = 0.358
